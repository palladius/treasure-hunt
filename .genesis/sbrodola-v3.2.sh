#!/bin/bash
# sbrodola.sh - Generates the Treasure Hunt Game Rails application (v3.2 - FAST_SKIP_STUFF)
...
APP_NAME="treasure-hunt-game-v32"
RUBY_VERSION="3.4.4"
...
log "Starting ${APP_NAME} Rails App Generation (v3.2 - FAST_SKIP_STUFF=${FAST_SKIP_STUFF})..."
...
if [ "$FAST_SKIP_STUFF" = "true" ]; then
  ...
  # Ensure sqlite3 gem is present if not added by `rails new --database=sqlite3` (it should be)
  if ! grep -q "gem 'sqlite3'" Gemfile && ! grep -q 'gem "sqlite3"' Gemfile; then
    bundle add sqlite3 --version "~> 1.7" # Or appropriate version
  fi
else
  ...
  # pg gem should be handled by `rails new --database=postgresql` if not FAST_SKIP_STUFF
fi
...
# --- Update Migrations for Indexes and Defaults ---
log "Updating migrations for indexes and default values..."
get_latest_migration_file() { ls db/migrate/*_$1.rb | tail -n 1; }

GAME_MIGRATION_FILE=$(get_latest_migration_file "create_games")
info "Updating migration: ${GAME_MIGRATION_FILE}"; sed -i.bak "/t.string :public_code/a\\    add_index :games, :public_code, unique: true" "$GAME_MIGRATION_FILE"; sed -i.bak 's/\(t.boolean :published\)/\1, default: true/' "$GAME_MIGRATION_FILE"; rm -f "${GAME_MIGRATION_FILE}.bak"
CLUE_MIGRATION_FILE=$(get_latest_migration_file "create_clues")
info "Updating migration: ${CLUE_MIGRATION_FILE}"; sed -i.bak "/t.references :game, null: false, foreign_key: true/a\\    add_index :clues, [:game_id, :unique_code], unique: true\\    add_index :clues, [:game_id, :series_id], unique: true" "$CLUE_MIGRATION_FILE"; sed -i.bak 's/\(t.boolean :published\)/\1, default: true/' "$CLUE_MIGRATION_FILE"; rm -f "${CLUE_MIGRATION_FILE}.bak"
PLAYER_PROGRESS_MIGRATION_FILE=$(get_latest_migration_file "create_player_progresses")
info "Updating migration: ${PLAYER_PROGRESS_MIGRATION_FILE}"; sed -i.bak "/t.string :player_token/a\\    add_index :player_progresses, :player_token, unique: true" "$PLAYER_PROGRESS_MIGRATION_FILE"; sed -i.bak 's/\(t.integer :current_clue_series_id\)/\1, default: 1/' "$PLAYER_PROGRESS_MIGRATION_FILE"; rm -f "${PLAYER_PROGRESS_MIGRATION_FILE}.bak"
...
if grep -q "t.boolean :is_admin" "$DEVISE_USERS_MIGRATION_FILE"; then if ! grep -q "t.boolean :is_admin, default: false" "$DEVISE_USERS_MIGRATION_FILE"; then sed -i.bak 's/\(t.boolean :is_admin\)/\1, default: false, null: false/' "$DEVISE_USERS_MIGRATION_FILE"; else info "'is_admin' already has default."; fi; else warn "Could not find 't.boolean :is_admin' in Devise migration."; fi; rm -f "${DEVISE_USERS_MIGRATION_FILE}.bak"
...
# --- Configure Routes --- (Content identical to v3.1)
...
# --- Create ricclib/color.rb --- (Content identical to v3.1)
...
# --- Create justfile --- (Content mostly identical, adjusted Docker notes)
...
setup: ; @echo "📦 Deps..."; bundle install; @echo "⚙️ DB prep..."; rails db:prepare; @echo "🌱 Seed..."; rails db:seed; @echo "✅ Done!"
...
{{if [ "${FAST_SKIP_STUFF:-false}" = "false" ]}}
test: ; @echo "🧪 RSpec..."; bundle exec rspec
{{else}}
test: ; @echo "🧪 Tests skipped in FAST_SKIP_STUFF mode."
{{endif}}
...
run-docker:
    @echo "Run Docker (expects PostgreSQL setup from FAST_SKIP_STUFF=false)..."
...
# Conditional lint command
{{if [ "${FAST_SKIP_STUFF:-false}" = "false" ]}}
lint: ; if bundle exec rubocop --version > /dev/null 2>&1; then bundle exec rubocop || echo "RuboCop issues."; else echo "RuboCop missing (not added in FAST_SKIP mode)."; fi
lint-fix: ; if bundle exec rubocop --version > /dev/null 2>&1; then bundle exec rubocop -A || echo "RuboCop fix."; else echo "RuboCop missing (not added in FAST_SKIP mode)."; fi
{{else}}
lint: ; @echo "Linting skipped in FAST_SKIP_STUFF mode."
lint-fix: ; @echo "Linting skipped in FAST_SKIP_STUFF mode."
{{endif}}
EOF

# --- Create .env.dist --- (Content identical to v3.1)
...
# --- Populate Model Files --- (Content identical to v3.1, using minified versions)
...
# --- Application & Other Controllers --- (Content identical to v3.1, using minified versions)
...
# --- Locales for languages --- (Content identical to v3.1)
...
# --- Database Seeds --- (Content identical to v3.1, using minified version)
...
bundle exec rails db:prepare # This will also run db:migrate if the DB is already created but has pending migrations.
...
if ! git rev-parse --is-inside-work-tree > /dev/null 2>&1; then
  ...
  git init -b main && git add . && git commit -m "🎉 Initial project: ${APP_NAME} by sbrodola.sh (v3.2)"
